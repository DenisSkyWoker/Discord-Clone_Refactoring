...
export async function verifyEmail() {
    ...
    if (!code || code.length !== 6 || !/^[0-9]{6}$/.test(code)) {
        if (errorEl) {
            errorEl.innerText = 'Введите 6-значный код';
    ...
    try {
        ...
        if (response.ok) {
            if (successEl) {
                successEl.innerText = 'Email успешно подтверждён!';
...
export async function resendCode() {
    ...
    try {
        ...
        if (response.ok) {
            if (successEl) {
                successEl.innerText = 'Код отправлен на ваш email';
                successEl.style.display = 'block';
            }
            setTimeout(() => {
            ...
            if (errorEl) {
                errorEl.innerText = data.detail || 'Ошибка отправки';
                errorEl.style.display = 'block';
            }
        }
    } catch (error) {
